//ЕСЛИ ЕСТЬ ЗАПУЩЕННЫЙ REST СЕРВИС, КОТОРАЫЙ РАБОТАЕТ С JSON, НАПРЯМУЮ ДОСТУПА НЕТ
class ItemService {
  constructor() {
    this.ITEM_COLLECTION_URL = "https://12x87.codesandbox.io/news.json";
  }
  async retrieveItems() {
    return fetch(this.ITEM_COLLECTION_URL)
      .then(response => {
        if (!response.ok) {
